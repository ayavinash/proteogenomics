...
### Assign gencode
    ...
    gencode_peptides = set(gencode_proteins.peptide.drop_duplicates())
    data.pgp[(data.pgp.notnull()) & (data.pep_seq.isin(gencode_peptides))] ="gencode_pep"
    current = data[data.pgp=="gencode_pep"]
...
### Assign refseq
    ...
    data.pgp[(data.pgp.isnull()) & (data.pep_seq.isin(set(refseq.peptide)))] ="refseq_pep"
...
## Assign uniprot
    ...
    uniprot["pep_locus"] = "NA"

    data.pgp[(data.pgp.isnull()) & (data.pep_seq.isin(set(uniprot.peptide)))] ="uniprot_pep"
...
###Assign exonskip
    ...
    data.pgp[(data.pgp.isnull()) & (data.pep_seq.isin(set(exonskip.peptide)))]="exonskip_pep"
...
### Assign gnomon novel isoform from ref assembly
    ...
    ref =ref[(ref.tryptic_match=="Yes") & (ref.assigned_in_main)  & (ref.refseq_gene_name_biotype.notnull())]
    ...
    data.pgp[(data.pgp.isnull()) & (data.pep_seq.isin(set(ref.peptide)))]="novel_isoform_pep"
...
### Assign gnomon novel isoform from alt assembly
    ...
    alt =alt[(alt.tryptic_match=="Yes") & (alt.assigned_in_main)  & (alt.refseq_gene_name_biotype.notnull())]
    ...
    data.assembly[(data.pgp.isnull()) & (data.pep_seq.isin(set(alt.peptide)))] = "alt"
    data.pgp[(data.pgp.isnull()) & (data.pep_seq.isin(alt.peptide))]="novel_isoform_pep"
...
### Assign gnomon novel isoform from ref assembly in haplotypes
    ref = ref_orig.copy(deep=True)
    ref = ref[~ref.peptide.isin(blast_mapped_peptides)]
    ref =ref[(ref.tryptic_match=="Yes") & ~(ref.assigned_in_main)  & (ref.refseq_gene_name_biotype.notnull())]
    ref["protein_coding_genes_biotypes"] = ref.refseq_gene_name_biotype.str.split(";")
...
### Assign gnomon novel isoform from alt assembly in haplotypes
    alt = alt_orig.copy(deep=True)
    alt = alt[~alt.peptide.isin(blast_mapped_peptides)]
    alt =alt[(alt.tryptic_match=="Yes") & ~(alt.assigned_in_main)  & (alt.refseq_gene_name_biotype.notnull())]
    alt["protein_coding_genes_biotypes"] = alt.refseq_gene_name_biotype.str.split(";")
...
### Assign gnomon non coding from ref assembly
    ...
    data.pgp[(data.pgp.isnull()) & (data.pep_seq.isin(set(ref.peptide)))]="non_coding_pep"
...
### Assign gnomon non coding from alt assembly 
    ...
    data.assembly[(data.pgp.isnull()) & (data.pep_seq.isin(set(alt.peptide)))] = "alt"
    data.pgp[(data.pgp.isnull()) & (data.pep_seq.isin(alt.peptide))]="non_coding_pep"
